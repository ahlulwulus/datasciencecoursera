...
classBMI<-function(bmic) {
        if (is.na(bmic)) return ("")
                ...
                if (bmic<18.5) return("You are considered UNDERWEIGHT") 
                if (bmic>=8.5 & bmic<24.9) return("You are in HEALTHY WEIGHT RANGE")
                if (bmic>=25 & bmic<29.9) return("You are considered OVERWEIGHT")
                if (bmic>=30) return("You are considered OBESE")
        }
}
